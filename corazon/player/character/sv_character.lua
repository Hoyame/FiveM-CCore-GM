...
AddEventHandler('corazon_character:savePed', function(sexeData, charID)
    local charID = charID
    local source = source
	local license = GetPlayerIdentifiers(source)[1]

	MySQL.Async.execute('UPDATE joueurs_d'..charID..'_perso SET `dataPed` = @sexeData WHERE license = @license', {
		['@sexeData'] = sexeData,
		['@license'] = license
	})
end)
